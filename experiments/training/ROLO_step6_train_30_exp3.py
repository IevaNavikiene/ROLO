...
class ROLO_TF:
    ...
    def training(self, x_path, y_path):
        ...
        with tf.Session() as sess:
            ...
            while step * self.batch_size < self.training_iters:
                ...
                batch_xs = self.rolo_utils.load_yolo_output(x_path, self.batch_size, self.num_steps, step) # [num_of_examples, num_input] (depth == 1)
                print('len(batch_xs)= ', len(batch_xs))
                # for item in range(len(batch_xs)):

                batch_ys = self.rolo_utils.load_rolo_gt(y_path, self.batch_size, self.num_steps, step)
                ...
                utils.save_rolo_output(self.output_path, pred_location, step, self.num_steps, self.batch_size)
                ...
                if step % self.display_step == 0:
                    ...
                    if self.disp_console: print "Iter " + str(step*self.batch_size) + ", Minibatch Loss= " + "{:.6f}".format(loss) #+ "{:.5f}".format(self.accuracy)
                    total_loss += loss
                step += 1
                if self.disp_console: print(step)
    ...
    def train_30(self):
        print("TRAINING ROLO...")
        log_file = open("output/trainging-30-log.txt", "a") #open in append mode
        self.build_networks()
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epoches):
                ...
                self.createFolder(self.output_path)
                total_loss = 0
                step = 0

                # Keep training until reach max iterations
                num_iters= self.training_iters * 3 / self.num_steps
                print num_iters
                while step * self.batch_size < num_iters:
                    # Load training data & ground truth
                    batch_xs = self.rolo_utils.load_yolo_output(x_path, self.batch_size, self.num_steps, step) # [num_of_examples, num_input] (depth == 1)
